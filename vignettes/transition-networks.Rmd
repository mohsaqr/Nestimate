...
```{r tna-plot, eval = requireNamespace("cograph", quietly = TRUE)}
library(cograph)
splot(net_tna)
```
...
```{r ftna-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_ftna)
```
...
```{r atna-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_atna)
```
...
```{r onehot-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net, layout = "oval")
```
...
```{r wtna-freq-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_wtna)
```
...
```{r wtna-relative-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_wtna_rel)
```

```{r wtna-mixed, eval = requireNamespace("cograph", quietly = TRUE)}
net_wtna_mixed <- wtna(learning_activities, method = "both", type = "relative")
print(net_wtna_mixed)
splot(net_wtna_mixed)
splot(net_wtna_mixed, type = "group")
```
...
```{r bootstrap-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(boot)
```
...
```{r perm-clusters, cache = TRUE}
perm <- permutation_test(Clusters$`Cluster 1`, Clusters$`Cluster 2`)
perm
```

```{r perm-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(perm)
```
...
```{r mmm-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(Mnets)
```
...
```{r posthoc-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(Postgr)
```
...
```{r cor-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_cor)
```
...
```{r pcor-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_pcor)
```
...
```{r glasso-plot, eval = requireNamespace("cograph", quietly = TRUE)}
splot(net_glasso)
```
