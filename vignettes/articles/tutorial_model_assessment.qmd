...
- **Split-half reliability.** If I randomly split my cases into two halves and estimate the network on each half, do the two networks agree? This is the classical internal-consistency check (Epskampet al., 2018).
...
- **Centrality case-dropping stability.** Same case-dropping procedure, but now we ask whether the *ranking* of nodes by centrality is preserved. The formal summary is the CS-coefficient (Epskampet et al., 2018).
...
`ai_long` is a bundled example dataset in `Nestimate` containing **8,551 coded actions from AI-side turns** across a human–AI vibe-coding study. Each row represents a single action, tagged with `session_id` and `timestamp`, and `code` indicates one of 8 actions (`Delegate`, `Plan`, `Execute`, `Investigate`, `Repair`, `Ask`, `Explain`, `Report`).
...
table(ai_long$code)
```
The most common actions are `Execute` and `Investigate`; `Ask` and `Report` are rare. That imbalance is one reason we want stability checks — rare codes produce unreliable edges because there are few observations of them.
...
The `edge_stab$summary` data frame gives the mean correlation at each drop proportion, which is useful for seeing *where* the stability breaks down rather than just the single summary.
