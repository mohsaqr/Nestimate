...
`cluster_data()` distance — `hamming`, `osa`, `lv`, `dl`, `qgram`,
`cosine`, `jaccard`, `jw`.
...
```{r i-cluster}
cl <- cluster_data(as.data.frame(trajectories), k = 3L,
                   dissimilarity = "hamming", method = "ward.D2")
sequence_plot(cl, type = "index",
              main = "index faceted by cluster_data(k = 3)")
```

### 2.4 Force a 1×3 row

```{r i-row, fig.width=14, fig.height=5}
...
cl <- cluster_data(as.data.frame(trajectories), k = 3,
                   dissimilarity = "hamming", method = "ward.D2")
sequence_plot(cl, type = "index")
sequence_plot(cl, type = "distribution")
