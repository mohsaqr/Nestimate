...
## 7. Sensitivity to `min_freq`

Raising `min_freq` cleans up the signal by dropping rare patterns; lowering it digs deeper into the tail.

### Stricter: `min_freq = 50`

```{r min50, fig.height=7}
res50 <- sequence_compare(net, sub = 3:5, min_freq = 50L,
                           test = "chisq", adjust = "fdr")
cat(sprintf("patterns: %d\n", res50$n_patterns))
plot(res50, style = "heatmap", top_n = 10)
```

### Looser: `min_freq = 10`

```{r min10, fig.height=9}
res10 <- sequence_compare(net, sub = 3:5, min_freq = 10L,
                           test = "chisq", adjust = "fdr")
cat(sprintf("patterns: %d\n", res10$n_patterns))
plot(res10, style = "heatmap", top_n = 15)
```

## 8. Note on the test choice
