...
#   sequence_compare()     vs tna::compare_sequences()
...
# ---- 7. sequence_compare: frequencies/proportions vs tna -------------------

test_that("sequence_compare: per-pattern freq/prop match tna across 20 configs", {
  skip_on_cran()
  skip_equiv_tests()
  skip_if_not_installed("tna")
  ...
  invisible(lapply(seq_len(N), function(i) {
    ...
    nest_cs <- tryCatch(
      sequence_compare(gd, group = "group", min_freq = 1L),
    ...
    report$log("sequence_compare", lbl, n_checked, 0L, 0, 0, 0, 0,
...
test_that("tna functions equivalence report", {
  ...
  report$write_cvs("tna_functions")
  msg <- report$summary()
  message(msg)
