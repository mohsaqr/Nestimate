...
- **NEWS.md**: added `# Nestimate 0.5.1` entry above the now-released
  ...
  changelog.
- **README.md + `_pkgdown.yml`**: replaced 12
  `mohsaqr.github.io/Nestimate/` URLs with `saqr.me/Nestimate/` — the
  GitHub Pages URLs were 200-OK but redirected, which CRAN’s URL check
  now flags as a NOTE.
...
- **`R CMD check --as-cran`**: clean. 0 errors, 0 warnings, 1
  ...
  examples (25s/26s OK), the full testthat suite (10s/13s OK), vignette
  re-build (8s/10s OK).
- **pkgdown**: site builds successfully via
  ...
  `RBGL` — pulled in by the `cograph -> gimme/lavaan` chain when pkgdown
  loads the namespace for S3 method overrides). Quarto must be on PATH
  (RStudio’s bundled quarto at
  `/Applications/RStudio.app/Contents/Resources/app/quarto/bin` works).
  Verified all audit-driven roxygen edits made it into the rendered HTML
  (`compare_mmm` `return_fits`, `build_mmm` “Initial states” section,
  `build_mcml` `Limitation:` note on edge-list cluster column,
  `build_clusters` “Missing-value distance rule” subsection,
  `summary.mcml` corrected return doc, `as_tna.mcml` corrected drop
  warning).
...
- `R/cluster_data.R`:
  ...
  distance branch now splits caller `...` between
  [`build_clusters()`](https://saqr.me/Nestimate/reference/build_clusters.md)
  args (`na_syms`, `weighted`, `lambda`, `seed`, `q`, `p`, `covariates`)
  and
  [`build_network()`](https://saqr.me/Nestimate/reference/build_network.md)
  args. Split runs on caller dots only; `data$build_args` is merged into
  the build_network side after, so attention-method (`atna`) netobjects’
...
- `R/mcml.R`: corrected stale
  [`summary.mcml()`](https://saqr.me/Nestimate/reference/summary.mcml.md)
  roxygen (audit_mcml \#5), documented edge-list `clusters = "<col>"`
  narrow contract (audit_mcml \#2), steered `method` doc toward `"sum"`
  for raw inputs (audit_mcml \#4), corrected
  [`as_tna.mcml()`](https://saqr.me/Nestimate/reference/as_tna.md)
  “Excluded Clusters” section to match the actual warning behaviour
  (audit_mcml \#6).
- `R/cluster_data.R`: documented NA-as-sentinel distance rule under
  `na_syms` (audit_clustering \#3).
- `R/mmm.R`: added “Initial states” section to
  [`build_mmm()`](https://saqr.me/Nestimate/reference/build_mmm.md)
  roxygen (audit_clustering \#5 doc-only path).

### Stage B — additive input validation (2 fixes)

- `R/cluster_data.R`:
  [`build_clusters()`](https://saqr.me/Nestimate/reference/build_clusters.md)
  now rejects all-missing input early (audit_clustering \#4) and uses
  kinder named-condition stopifnot
  - explicit [`stop()`](https://rdrr.io/r/base/stop.html) for k-range
    checks (audit_clustering \#2). Updated one existing test for new
    error text; +3 new tests.
...
- `R/mcml.R`:
  ...
  requires `node_groups` to carry a node identifier column (data.frame)
  or be a named atomic vector keyed by node label (audit_mcml \#1).
  Unnamed bare vectors rejected with a clear error. Existing test that
  pinned the buggy positional read updated to the new contract; +3 new
  tests including the misordered-input alignment proof and a
  label-propagation test through
...
- `R/mmm.R`: `compare_mmm(return_fits = FALSE)` new arg
  ...
  `attr(result, "fits")` keyed by k. Default behaviour unchanged. +4
  tests.

### Stage E — test gaps (no code change)

- `tests/testthat/test-mmm.R`: pinned first-column-NA init_state
  behavior and sentinel-character-as-real-state behavior. The first test
  caught a doc inaccuracy I’d written and was corrected in real time.
- `tests/testthat/test-mcml.R`: pinned the documented narrow contract
  ...
  that triggers the
  [`as_tna.mcml()`](https://saqr.me/Nestimate/reference/as_tna.md) drop
  warning (sequence input where one cluster has a node with no
  in-cluster outgoing transitions).
...
- Files changed: 4 R modules (`cluster_data.R`, `mcml.R`, `mmm.R`), 7
  regenerated man pages (`as_tna.Rd`, `build_clusters.Rd`,
...
(matrix path) symmetrizes the input matrix when `directed = FALSE`. -
[`build_mcml()`](https://saqr.me/Nestimate/reference/build_mcml.md) raw
data path does NOT apply symmetrisation before row-normalisation when
`directed = FALSE`. -
[`.process_weights()`](https://saqr.me/Nestimate/reference/dot-process_weights.md)
accepts `directed` but doesn’t actually use it.

The two coherent contracts are: - **(a) Make raw paths symmetrise too.**
Adds an extra step before normalisation; produces a symmetric within
matrix that’s then row-normalised. Numeric change for any existing
caller using `directed = FALSE` on raw data — though there might be
none, since the current behaviour is a silent no-op. - **(b) Document
`directed = FALSE` as matrix-aggregation-only.** Steer raw-data users to
`type = "cooccurrence"` for undirected summaries. No numeric change.
Lighter touch but leaves the contract fragmented.
...
verbatim” to “first non-NA column per sequence”. Effect: - Sequences
whose first column is NA would enter EM with a real init rather than
NA. - Could change EM convergence trajectories on real data, especially
ESM/EMA panels with leading missings. - Need a deliberate decision
because the change is invisible from outside (no API change), but
downstream BIC/AIC/posteriors shift.
...
- **Risk-ordered staging.** Documentation first, additive checks second,
  contract changes third, additive features fourth, test gaps fifth.
  Each stage ran the full sweep before moving on. This caught one doc
  inaccuracy in Stage E that wouldn’t have surfaced with a bottom-up
  “fix everything then test” approach.
- **Rejected the named-vector-with-cluster-col-name path in
  [`.auto_detect_clusters()`](https://saqr.me/Nestimate/reference/dot-auto_detect_clusters.md).**
  A “named vector” already uses names() for node labels — there’s no
  separate “cluster column” concept for that shape. Restructured the
  function to dispatch on
  [`is.data.frame()`](https://rdrr.io/r/base/as.data.frame.html) /
  [`is.atomic()`](https://rdrr.io/r/base/is.recursive.html) first, then
  use the right accessor in each branch.
- **Updated rather than preserved a pre-existing test that pinned the
  ...
  contract is the right move; preserving it via a back-compat shim would
  have re-introduced the corruption hazard.
- **Attached fits as an attribute, not a column.**
  [`compare_mmm()`](https://saqr.me/Nestimate/reference/compare_mmm.md)’s
  ...
  invisible by default and explicit when wanted via
  `attr(comp, "fits")[[k]]`.
...
- Audit reports for reference: `codex_docs/audit_clustering/report.Rmd`
